...
public class PongBallSprite implements DisplayableSprite {
	...
	public PongBallSprite(double centerX, double centerY, double velocityX, double velocityY) {
		...
		collisionTargetTypes.add(PongBarrierSprite.class);
		collisionTargetTypes.add(PongSpriteLeft.class);
		collisionTargetTypes.add(PongSpriteRight.class);
		collisionTargetTypes.add(PongBallSprite.class);
		...
		if (image == null) {
			try {
				image = ImageIO.read(new File("res/simpleTile.png"));
			}
			catch (IOException e) {
	...
	public void update(Universe universe, KeyboardInput keyboard, long actual_delta_time) {
		...
		//(AnimationFrame.SCREEN_WIDTH/2)                 - the y line of left
		if (getMinX() < (50 - (AnimationFrame.SCREEN_WIDTH/2))) {
	        ((PongUniverse) universe).incrementRightPlayerScore();
	        resetBall();
	    }

	    // (AnimationFrame.SCREEN_WIDTH/2) -50         - the y line of right
	    if (getMaxX() > ((AnimationFrame.SCREEN_WIDTH/2) -50)) {
	        // The ball crossed the right boundary, increment the left player's score
	        ((PongUniverse) universe).incrementLeftPlayerScore();
	...
	private void resetBall() {
	    this.centerX = 0; // Set to the center of the screen or your desired initial position
	    this.centerY = 0; // Set to the center of the screen or your desired initial position
	    // You might also want to reset velocity and acceleration if needed
